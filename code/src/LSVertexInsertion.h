...
public:		
        ...
        /// <returns></returns>
        static int VertexInsertion(Graph &g, SteinerSolution &solution, int maxv) {
            int n = g.VertexCount();
            int v = 0;
			...
			for (int e=1; e<=m; e++) { // looping over all edges (could be more efficient)
                ...
                if (debug_mode && (!svertices.Contains(a) || !svertices.Contains(b))) ;
			...
			while (failures < n) {
				...
				for (g.GetBounds(v,a,end); a<end; a++) {
					...
					if (first) { //first edge: just insert it
						...
						if (debug_mode && (dyntree.GetRoot(v) == dyntree.GetRoot(w))) ;
                        ...
                        if (zcost > cost) {
                            ...
                            if (debug_mode && (dyntree.GetRoot(v) != v)) ;
